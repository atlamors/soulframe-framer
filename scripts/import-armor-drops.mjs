...
const OUTPUT_PATH = resolve("src/data/armor-drops.generated.json");
...
const armorByName = new Map(catalogue.map((item) => [item.name, item]));
const sourcesByItemId = new Map(catalogue.map((item) => [item.id, []]));
...
for (const table of extractAssignments(moduleSource)) {
  ...
  for (const dropBody of extractDirectObjects(
    ...
    const armor = armorByName.get(itemName);
    if (!armor) continue;

    for (const source of sourceRows) {
      sourcesByItemId.get(armor.id).push({
        tableId: table.id,
        category: tags,
...
console.log(
  `Mapped ${output.coverage.totalSourceRows} drop-source rows across ${sourcedItems.length}/${catalogue.length} armour items from Avakot revision ${sourceRevision.revisionId}.`,
);
