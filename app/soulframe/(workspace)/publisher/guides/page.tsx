...
export default async function GuidesPublisherPage() {
  const { auth, publications } = await getBackendForRequest();
  const session = await auth.getSession();
  if (!session) redirect("/auth/sign-in?next=/soulframe/publisher/guides");
  ...
  const owned = (await publications.listOwned({ ownerId: session.account.id }))
    .filter((publication) => publication.profileId === "soulframe.guide");
  ...
  return (
    <main className="min-h-[calc(100vh-5rem)] px-4 py-8 text-ink sm:px-6">
      <div className="mx-auto max-w-5xl">
        <header className="flex flex-wrap items-end justify-between gap-4 border-b border-line/60 pb-5">
          <div>
            ...
            <p className="mt-2 font-sans text-sm text-ink-soft">
              Draft, publish, and update long-form Soulframe guides.
            </p>
          </div>
          <Link
            href={profile?.publisherEligibility.eligible ? "/soulframe/publisher/guides/new" : "/soulframe/profile?next=/soulframe/publisher/guides/new"}
        ...
        <section aria-label="Your guides" className="mt-5 grid gap-3">
          {owned.length ? (
            owned.map((publication) => (
              <Link
                key={publication.id}
                href={`/soulframe/publisher/guides/${publication.id}`}
              ...
              >
                <span className="min-w-0">
                  <span className="block truncate font-display text-lg text-gold-bright">
                    {publication.draft.state.metadata.title || "Untitled guide"}
                  </span>
            ...
            <div className="rounded-md border border-dashed border-line/60 p-8 text-center font-sans text-sm text-ink-muted">
              No guides yet. Create one when you are ready.
            </div>
          )}
        </section>
      </div>
